...
class BpeTokenizer(Tokenizer):
    ...
    def train(self, text, vocab_size, verbose=False):
        ...
        for i in range(num_merges):
            ...
            idx = 256 + i
            ids = [merge_pairs(chunk,pair, idx) for chunk in chunk_ids]
            merges[pair] = idx
    ...
    def _encode_chunk(self, text_bytes:bytes):
        ...
        while len(ids) >=2:
            stat = get_pairs(ids)
            pair = min(stat, key=lambda p: self.merges.get(p, "inf"))
    ...
    def encode(self, text, allow_special="none_raise"):
        ...
        if allow_special == 'all':
            ...
            special_tokens = {}
            all(token not in text for token in self.special_tokens)
        elif isinstance(allow_special, set):
            special_tokens = {k:v for k,v in self.special_tokens.items() if k in allow_special}
        else:
            raise ValueError(f"allow_specal={allow_special} is not valid")
