...
def bpe(mergeable_ranks:dict, token:bytes, max_token:int=None):
    ...
    # reasoning behind parts:
    # token = b'hi':
    # [b for b in token] outputs [104, 105].
    # [bytes([b]) for b in token] outputs [b'h', b'i'].
    while True:
        min_idx = None
        min_rank = None
...
class GPT4Tokenizer(BpeTokenizer):
    def __init__(self):
        ...
        # and probably historical[Andrej Kaparthy], but therefore we have to deal with it here.
