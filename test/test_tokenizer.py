# this test tokenizer file is copied verbatim from the minBPE repo.
# link: https://github.com/karpathy/minbpe/blob/ade909f8af2e9534ea3d2f37f68e96e9703ebcac/tests/test_tokenizer.py

import pytest
import tiktoken
import os

from bpe import minBPE, BPETokenizer, GPT4Tokenizer
# -----------------------------------------------------------------------------
# common test data
...
test_strings = [
    ...
    "FILE:pg_essays.txt", # FILE: is handled as a special string in unpack()
]
def unpack(text):
    ...
    if text.startswith("FILE:"):
        ...
        pg_essays_file = os.path.join(dirname, text[5:])
        contents = open(pg_essays_file, "r", encoding="utf-8").read()
        return contents
    else:
        return text
...
@pytest.mark.parametrize("tokenizer_factory", [minBPE, BPETokenizer, GPT4Tokenizer])
@pytest.mark.parametrize("text", test_strings)
def test_encode_decode_identity(tokenizer_factory, text):
    text = unpack(text)
    tokenizer = tokenizer_factory()
    kwargs = {}
    if tokenizer_factory in [BPETokenizer, GPT4Tokenizer]:
...
@pytest.mark.parametrize("tokenizer_factory", [minBPE, BPETokenizer])
